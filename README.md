...
| File | Purpose |
|------|---------|
| `esp32s3box3.yaml` | Main ESPHome configuration (~4 300 lines) |
...
In `esp32s3box3.yaml`, update the substitution near the top:
...
```bash
esphome compile esp32s3box3.yaml
```

### 4. Flash (first time — USB-C)

```bash
esphome run esp32s3box3.yaml
```

### 5. OTA updates (once on Wi-Fi)
